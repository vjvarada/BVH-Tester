	...
	runTask( worker, geometry, options = {} ) {
		...
		return new Promise( ( resolve, reject ) => {
			...
			worker.onerror = e => {
				...
				const msg = e && ( e.message || ( e.error && e.error.message ) || e.type ) || 'Unknown error';
				console.error( 'GenerateMeshBVHWorker worker error:', e );
				reject( new Error( `GenerateMeshBVHWorker: ${ msg }` ) );
